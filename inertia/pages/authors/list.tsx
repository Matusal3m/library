...
export default function ListAuthors(props: InferPageProps<AuthorsController, 'index'>) {
  return (
    <ul className="grid grid-cols-3 gap-4 p-2">
      {props.authors.map((author) => (
        <Link href={`/authors/${author.id}`} key={author.id}>
          <li className="block p-6 bg-white border border-gray-200 rounded-lg shadow-sm hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
